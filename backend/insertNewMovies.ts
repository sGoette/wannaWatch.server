...
export const insertNewMovies = (database: DatabaseSync, MOVIE_LOCATION: string, MOVIE_THUMBNAIL_LOCATION: string, libraryId?: number): Promise<void | MovieMetadata>[] => {
    ...
    libraries.filter(library => !libraryId || library.id === libraryId).forEach(async library => {
        ...
        promisses.concat(movies.map(async movie => {
            ...
            insertMovie.run(title, file_location, metaData.duration, metaData.width ?? 0, metaData.height ?? 0, metaData.codec ?? "", metaData.format ?? "", metaData.thumbnailPath ?? "", library.id)
            database.close()
        }))
    })
